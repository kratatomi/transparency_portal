...
if __name__ == "__main__":
    ...
    # SIDX smart contract address on SmartBCH
    ...
    class JSONifiedState(EventScannerState):
        ...
        def __init__(self):
            self.state = None
            self.fname = "data/balances/transfer_events.json"
        ...
        def process_event(self, block_when: datetime.datetime, event: AttributeDict) -> str:
            ...
            args = event["args"]
            # Convert Transfer event to our internal format
		...
		# Create empty dict as the block that contains all transactions by txhash
            	...
            	self.state["blocks"][block_number][txhash][event_name] = transfer
    ...
    def run():
        ...
        if len(sys.argv) < 2:
            print("Usage: sbch_eventscanner.py http://your-node-url")
            sys.exit(1)
        ...
        # Assume we might have scanned the blocks all the way to the last SmartBCH block
        # that mined a few seconds before the previous scan run ended.
        # Because there might have been a minor SmartBCH chain reorganisations
